...
- Lock-in>Sync Filter D2	ON	#  Switch the Sync filter on the demodulated signals (X,Y) on or off for D2. The sync filter operates after the low-pass filter and averages the demodulated signals over one period of the modulation frequency. 
